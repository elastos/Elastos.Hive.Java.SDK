...
public class RemoteResolver implements TokenResolver {
	...
	public RemoteResolver(ServiceEndpoint serviceEndpoint) {
		contextProvider = serviceEndpoint.getAppContext().getAppContextProvider();
		controller = new AuthController(serviceEndpoint);
	}

	@Override
	public AuthToken getToken() throws HttpFailedException {
		try {
			String accessToken = controller.auth(signIn());
